...
`core.required_facts` asserts that a canonical field is present and known. `status_in` optionally restricts its evidence status; `temporal_status_in` independently restricts it to `current`, `historical`, `resolved`, or `unknown`. `numeric` optionally checks the extracted numeric value within an absolute tolerance. `core.timeline_counts` requires a minimum number of retained observations for a field, which is useful for cross-document conflicts. `forbidden_facts` rejects invented known facts, while `forbidden_values` rejects specific unsafe values without forbidding a supported alternative or a conflict status.
