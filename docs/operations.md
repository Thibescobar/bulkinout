...
The interactive form listens on `127.0.0.1` at a random port, uses a single-use high-entropy URL token, accepts a bounded form body, and loads no remote assets. The operator has ten minutes to submit all currently known questions. The same HTTP response remains pending during Request recalculation, returns the final handoff in that page, and only then closes the local server. These controls limit accidental exposure; they do not authenticate the declared clinician, protect a compromised workstation, or create a durable workflow session.
