...
The interactive form listens on `127.0.0.1` at a random port, uses a single-use high-entropy URL token, accepts a bounded form body, loads no remote assets, and closes after submission or ten minutes. These controls limit accidental exposure; they do not authenticate the declared clinician, protect a compromised workstation, or create a durable workflow session.
